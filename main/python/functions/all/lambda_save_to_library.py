...
def lambda_handler(event, context):
    try:
        ...
        try:
            user_library_table.get_item(Key={'id': user_id})["Item"]
        except:
            user_library_table.put_item(Item={'id': user_id, 'tracks': []})

        user_library = user_library_table.get_item(Key={'id': user_id})["Item"]
        if track_id not in list(user_library["tracks"]):
            user_library_table.update_item(
                Key={'id': user_id},
